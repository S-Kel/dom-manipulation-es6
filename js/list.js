	...
	static createElement(text, index) {
		const card =
			`<div class="col-md-4 cards"> 
			 <div class="card">
				<img class="card-img-top card-img" src="${'https://picsum.photos/' + (100 + index) + '/100/?random'}" alt="Card image cap">				
				<div class="card-body">
				  <h5 class="card-title">${List.capitalize(text)}</h5>				  
				  <p class="card-text">
					<small class="text-muted">
						Last updated ${List.timeAgo(new Date(Date.now()))}			
					</small>					
				  </p>				  
				</div>	
				<button onClick="removeElement(this)" class='btn btn-link btn-sm btn-props ml-auto'>X</Button
			 </div>	
		    </div>`;
		return card;
	}
